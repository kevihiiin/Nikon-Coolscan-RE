...
def test_execute_type_calls_typestring(mock_inner: MagicMock) -> None:
    c = _client_with(mock_inner)
    c.execute(Type(type="type", text="hello"))
    mock_inner.typeString.assert_called_once_with("hello")


def test_execute_key_calls_keypress(mock_inner: MagicMock) -> None:
    c = _client_with(mock_inner)
